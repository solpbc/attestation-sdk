...
| Architecture has multiple list elements | `Apple architecture validation failed: CMAKE_OSX_ARCHITECTURES '${CMAKE_OSX_ARCHITECTURES}' contains ${_nvat_apple_architecture_count} entries, expected exactly one; remove the build directory and configure natively with -DCMAKE_OSX_ARCHITECTURES=arm64, then retry` |
